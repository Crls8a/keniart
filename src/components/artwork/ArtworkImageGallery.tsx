  ...
  return (
    <section aria-label={`Galería de imágenes de ${artwork.title}`} className="space-y-4">
      ...
      >
        ...
        {hasMultipleImages ? (
          <div className="pointer-events-none absolute inset-x-3 top-1/2 z-30 flex -translate-y-1/2 justify-between gap-3 sm:inset-x-5">
            <button
              aria-label="Ver imagen anterior"
              className="pointer-events-auto grid size-12 place-items-center rounded-full border border-line bg-paper/95 text-2xl shadow-[0_12px_30px_rgba(0,0,0,0.18)] backdrop-blur transition hover:bg-paper focus-visible:outline-offset-4 sm:size-14"
              onClick={() => showImage(active.index - 1, -1)}
